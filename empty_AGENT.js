...
function giveaninitialsequence(parent){
	...
	if(parent==undefined){
		for(let i=0;i<50;i++){
			...
			childpath.push(action);
		}
	}else{
		childpath=parent.actionHistory
		if(childpath.length>50){
			
			for(let i=0;i<5;i++){//random Mutations
				//console.log("Mutatet",i)
				childpath[Math.floor(Math.random()*childpath.length)]=possActions[Math.floor(Math.random()*(possActions.length-1))+1];

			}
			for(let i=0;i<(childpath.length);i++){//SpACEKILLER
				//console.log("space killed",i)
				if (childpath[i]==="space"){childpath[i]=possActions[Math.floor((Math.random()*(possActions.length-1))+1)];

				}
			}
			childpath.length=50;//caps the path size (not hard)
		}
