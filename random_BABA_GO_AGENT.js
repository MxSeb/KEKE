...
function newState(kekeState, map) {
  ...
  function giveaninitialsequence(parent){
	...
	if(parent==undefined){
		...
		if(parent.actionHistory.length>500){
			parent.actionHistory.length=500;
		}
		for(let i=0;i<(parent.actionHistory.length);i++){
			let action = parent.actionHistory[Math.floor(Math.random()*parent.actionHistory.length)];
			s.push(action);
		}
	}
	an_action_set = s;
